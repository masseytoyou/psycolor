...
def safe_text(value: Optional[str]) -> str:
    """Safely convert DB/DataFrame values to stripped text.

    Handles None, pandas NaN/NA, numeric IDs, and other non-string values.
    """
    if value is None:
        return ""
    try:
...
def load_received_messages(receiver_user_id: int) -> pd.DataFrame:
    """
    보고서 수신함은 Streamlit cache를 전혀 사용하지 않습니다.

    inbox_message.file_bytes는 PostgreSQL BYTEA이고 psycopg2/pandas 조합에 따라
    memoryview/object 형태로 들어올 수 있습니다. 이 값을 포함한 DataFrame을
    @st.cache_data가 직렬화하는 과정에서 UnserializableReturnValueError가
    반복적으로 발생했으므로, 수신함 조회는 매번 DB에서 직접 읽도록 합니다.
    """
    conn = get_connection()
    try:
        query = """
        ...
        # PostgreSQL BYTEA/memoryview를 일반 bytes로 정규화합니다.
